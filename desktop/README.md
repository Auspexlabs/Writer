...
1. **One window per folder.** A plain launch opens the drafts folder (`<app data>/Drafts`, created when missing): new documents live there as drafts until their first 存储, which opens the system Save dialog and grants the chosen file to that window's engine over `--grants-stdin` (另存为… asks again, leaving the original as it was). Opening a file — Finder double-click, a drop on the Dock icon, `open -a Writer file`, 文件 › 打开… (⌘O), 打开最近使用, the start page's own recent files, or a drop on a window — focuses the window for that file's folder, or opens one, and opens the file's tab there. A tab dragged off the title strip (let go outside the window or 40px below the strip, `ui/tabs.js`) takes its document to a new window at that point, also when another window has its folder: the page saves it first, `tear_off` opens the window, which shows only that document, and the tab closes. Every window runs `writer app --no-browser --list-depth <n> --dir <folder> --ui <resources>/ui --drafts <app data>/Drafts --grants-stdin` (depth 3 for the drafts folder, 0 for a file's folder; `src-tauri/src/main.rs`). Closing a window stops its engine; closing the last one leaves Writer in the Dock, ⌘Q quits.
...
9. **Updates** (`src-tauri/src/update.rs`, tauri-plugin-updater): 3 s after the first window of every launch is up and then once a day, unless 设置 › 通用 › 自动更新 is off, Writer reads `https://thewriter.cn/updates/mac/latest.json`, and `latest.json` from the newest GitHub release when that fails (GitHub downloads are slow in mainland China; `plugins.updater.endpoints` in `tauri.macos.conf.json` / `tauri.windows.conf.json`). A connection or TLS error or a non-2xx status moves on to GitHub; a 2xx reply that is not JSON ends the check with an error, and when the website serves a manifest, GitHub is not asked. A newer version downloads in the background and its signature is checked against `plugins.updater.pubkey`; the app bundle is replaced only when Writer quits, after the engines have stopped (a bundle replaced mid-run would pair this shell with a newer engine and UI), so the next launch runs it. This background download runs only when the user can write the bundle and its folder (no admin prompt); otherwise only Writer › 检查更新… downloads.
   **What the user sees** (`ui/UpdateNotice.dc.html`, which the shell imports; Rust calls `window.__writerUpdate` in the front document window only, or in the first one to report in): once an update is downloaded, a glass pill at the bottom, 「Writer 0.1.3 已准备好 · 查看更新内容 · 立即更新 · ×」. 查看更新内容 shows the release notes (`notes` of `latest.json` / `ui.json`), 立即更新 restarts into the update (`restart_app`: installed on the way out, then Writer opens again), × hides it and the update still installs at quit. The notes are kept in `<app data>/update.json` when the update is downloaded; the first launch that runs that version (or interface build) shows 「Writer 已更新到 0.1.3」 with them and 好, once. 检查更新… shows the pill again, or the toast 「Writer 0.1.2 已是最新版本」; with every window closed it opens one for the answer. A failed manual check ends in a dialog (前往下载页); an automatic one stays quiet.
10. **默认打开方式** (`src-tauri/src/default_app.rs`): on a version's first launch, when a double-click on .docx/.xlsx/.pptx/.md/.mm does not open Writer (LaunchServices per extension, `NSWorkspace`), the first document window shows a small sheet, 把 Writer 设为默认打开方式？: 设为默认 sets every type through `NSWorkspace`, 以后再说 waits for the next version (`<app data>/default-app.json`). 设置 › 通用 shows the state with 设为默认, and next to 自动更新 the version with 检查更新.
...
6. **Updates**: as on the Mac (item 9 above, with the website's `updates/windows/latest.json`; W › 检查更新… calls `check_update`), from the `windows-x86_64` entry of `latest.json`. The installer cannot replace a running Writer, so a new version is only downloaded and verified in the background; when Writer quits (`RunEvent::Exit`, after the engines stop, together with those of windows closed just before) the NSIS installer runs quietly (`plugins.updater.windows.installMode`) and does not reopen Writer. The notice's 立即更新 installs it and opens Writer again. Interface packages, the notice and What's New work as on the Mac.
7. **默认打开方式** (item 10 above): the check asks the shell which program opens each extension (`AssocQueryStringW`, the user's choice first). Windows lets no app make itself the default, so 设为默认 / 打开系统设置 opens 设置 › 默认应用 at Writer (`ms-settings:defaultapps?registeredAppUser=Writer`); `src-tauri/windows/hooks.nsh` (NSIS `installerHooks`) registers Writer in `Software\RegisteredApplications` with a `Capabilities\FileAssociations` list of the ProgIds Tauri's installer wrote, and removes it on uninstall.
