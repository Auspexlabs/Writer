...
(() => {
  ...
  // Writer perf patch: templates compile once, so their expressions do too. A plain dotted path (the common case)
  // becomes a lookup loop; anything else still goes through resolve() on every render.
  var PATH_RE = /^[A-Za-z_$][A-Za-z0-9_$]*(?:\.(?:[A-Za-z_$][A-Za-z0-9_$]*|\d+))*$/;
  var LITERALS = /* @__PURE__ */ new Set(["true", "false", "null", "undefined"]);
  function compileExpr(src) {
    const expr = String(src).trim();
    if (!PATH_RE.test(expr) || LITERALS.has(expr)) return (vals) => resolve(vals, expr);
    const segs = expr.split(".");
    return (vals) => {
      let cur = vals == null ? void 0 : vals[segs[0]];
      for (let i = 1; i < segs.length; i++) cur = cur == null ? void 0 : cur[segs[i]];
      return cur;
    };
  }
  ...
  var CSS_CACHE = /* @__PURE__ */ new Map();
  function cssToObj(css) {
    const hit = CSS_CACHE.get(css);
    if (hit) return hit;
    const o = {};
    ...
    if (CSS_CACHE.size > 2e4) CSS_CACHE.clear();
    CSS_CACHE.set(css, o);
    return o;
  }
  function compileAttr(raw) {
    const whole = raw.match(/^\s*\{\{([\s\S]+?)\}\}\s*$/);
    if (whole) return compileExpr(whole[1]);
    if (raw.includes("{{")) {
      const parts = raw.split(/\{\{([\s\S]+?)\}\}/g).map((s, i) => i & 1 ? compileExpr(s) : s);
      return (vals) => {
        let out = "";
        for (let i = 0; i < parts.length; i++) out += i & 1 ? String(parts[i](vals) ?? "") : parts[i];
        return out;
      };
    }
    return () => raw;
  ...
  function walkText(node) {
    ...
    const gets = parts.map((p, i) => i & 1 ? compileExpr(p) : null);
    // Writer patch: SVG draws no HTML <span>, so a value inside an SVG element (a chart's <text>) goes in as plain text.
    const inSvg = node.parentNode?.namespaceURI === "http://www.w3.org/2000/svg";
    return (vals, ctx, key) => h(
      ...
      ...parts.map((p, i) => {
        ...
        return inSvg ? String(v) : h("span", { key: i, className: "sc-interp" }, String(v));
      })
    );
  }
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      ...
      /** Writer perf patch (see ui/memo.js): skip the render when props and state are unchanged; registry bumps
       *  still arrive through forceUpdate. Without memo.js every render goes through, as the generated runtime did. */
      shouldComponentUpdate(nextProps, nextState) {
        const m = window.dcMemo;
        return !m || nextState !== this.state || !m.propsEqual(this.props, nextProps);
      }
      /** The ancestor chain handed to the context provider, kept identical while the parent's chain is, so an
       *  unchanged child is not force-updated by a new context value on every render. */
      __ancestors(chain) {
        if (this.__ancChain !== chain) {
          this.__ancChain = chain;
          this.__anc = [...chain, this.__name];
        }
        return this.__anc;
      }
      componentDidMount() {
